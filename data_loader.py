...
nltk.download('omw-1.4')      # Required by WordNetLemmatizer
...
if __name__ == "__main__":
    ...
    # Strategy for missing values (example for 'emails.csv'):
    # The 'emails.csv' often has several unnamed columns that are entirely empty.
    ...
    # For 'emails.csv', columns like 'Unnamed: 2', 'Unnamed: 3', 'Unnamed: 4' are often empty.
    ...
    # For 'emails.csv', the target column is usually 'v1'
    ...
    print("\n--- 4. Renaming Columns ---")
    # For 'emails.csv', 'v1' is typically the label and 'v2' is the message.
    ...
    # For 'emails.csv', after dropping empty 'Unnamed' columns and renaming,
    ...
    # Continuing from data_loader.py or in a new eda.py script

import pandas as pd
import os

# Assuming you have the load_dataset function defined as before:
def load_dataset(file_path):
    """
    Loads a dataset into a Pandas DataFrame, handling CSV and TSV formats.
    """
...
if __name__ == "__main__":
    ...
    csv_file_path = 'data/emails.csv' # Adjust this path
    try:
        df = load_dataset(csv_file_path)
        print("\nInitial DataFrame loaded successfully for EDA.")
    except Exception as e:
    ...
    # You can now save the cleaned DataFrame if you want, or pass it to the next stage
    # df.to_csv('data/sms_spam_collection_cleaned.csv', index=False)


# --- Your preprocess_text function (from before) ---
def preprocess_text(text, use_stemming=False, use_lemmatization=True):
    ...
    text = re.sub(r'\d+', '', text) # Removing numbers for now
